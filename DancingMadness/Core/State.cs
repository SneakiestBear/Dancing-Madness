    ...
    public sealed class State
    {
        ...
        public void LoadLocalTimelines(uint territory)
        {
            try
            {
                ...
                Regex rex = new Regex("DancingMadness_(?<territory>[0-9]{1,})[^0-9]");                
        ...
        public void Initialize()
        {
            ...
            halp.Add("Open Dancing Madness configuration");
            halp.Add("/umad clear → Force clear all markers");
            halp.Add("/umad mark <sign> <target> → Places a softmarker; follows the same arguments as the ingame /mk command");
#if !SANS_GOETIA
            halp.Add("/umad <feature> <on|off|toggle> → Turn or toggle feature on/off, where feature can be: automarkers, drawing, sound, hacks, automation, softmarkers");
#else
            halp.Add("/umad <feature> <on|off|toggle> → Turn or toggle feature on/off, where feature can be: automarkers, drawing, sound, softmarkers");
#endif
            cm.AddHandler("/umad", new CommandInfo(OnCommand)
            {
        ...
        internal bool StartDrawing(out ImDrawListPtr drawList)
        {
            ...
            ImGui.Begin("DancingMadnessCanvas",
