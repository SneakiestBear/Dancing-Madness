...
using static DancingMadness.Core.AutomarkerPrio;
using static DancingMadness.Core.State;
using Vector2 = System.Numerics.Vector2;
using Vector3 = System.Numerics.Vector3;
using Vector4 = System.Numerics.Vector4;

namespace DancingMadness
{
    ...
    public sealed class Plugin : IDalamudPlugin
    {
...
#if !SANS_GOETIA
        public string Name => "Dancing Madness Goetia";
#else
        public string Name => "Dancing Madness";
#endif
        ...
        public Plugin(IDalamudPluginInterface pluginInterface)
        {
            ...
            _state.plug = this;
            _ui._state = _state;
            Log(LogLevelEnum.Info, "This is Dancing Madness {0}", Version);
            LoadConfig();
            ApplyVersionChanges();
            ...
            _mainThread.Name = "Dancing Madness main thread";
        ...
        internal void DeserializeTimelineOverrides(string data)
        {
            ...
            doc.LoadXml(data);
            XmlNode root = doc.SelectSingleNode("/DancingMadness/TimelineOverrides");
            if (root == null)
            {
                return;
            }
        ...
        private void DeserializeProperties(string data)
        {
            ...
            XmlDocument doc = new XmlDocument();
            doc.LoadXml(data);
            XmlNode root = doc.SelectSingleNode("/DancingMadness");
        ...
        {
            XmlDocument doc = new XmlDocument();
            XmlNode root = doc.CreateElement("DancingMadness");
        ...
        public void BackupConfig()
        {
            ...
            string tempfile = Path.Combine(temp, String.Format("dancingmadness_backup_{0}.json", DateTime.Now.ToString("yyyyMMdd_HHmmssfff")));
...
#if !SANS_GOETIA
            temp = "DancingMadness2_" + temp;
#else
            temp = "DancingMadness1_" + temp;
#endif
            while (i < temp.Length)
            {
...
#if !SANS_GOETIA
            string mytag = "DancingMadness2_";
#else
            string mytag = "DancingMadness1_";
#endif
        ...
        private void TextToSpeech(string text)
        {
            try
            {
                ...
                string cmd =
                    ...
                    $speak.Speak(""{text}"");";
                string path = Path.Combine(Path.GetTempPath(), String.Format("DancingMadness_ttsscript_{0}.ps1", _ttsCounter));
        ...
        {
            Vector2 winSize;
            if (ImGui.Begin("DancingMadnessShortcut", ImGuiWindowFlags.NoDecoration | ImGuiWindowFlags.NoBackground | ImGuiWindowFlags.NoMove) == true)
            {
                ...
                IDalamudTextureWrap tw = _ui.GetMiscIcon(UserInterface.MiscIconEnum.DancingMadness).GetWrapOrEmpty();
                ...
                if (_movingShortcut == true)
                {
                    if (ImGui.IsMouseDown(ImGuiMouseButton.Right) == true)
                    {
                        ...
                        ImGui.SetWindowPos("DancingMadnessShortcut", new Vector2(cpos.X + delta.X, cpos.Y + delta.Y));
                    }
                    else
                    {
        ...
        private void GeneratePackage()
        {
            ...
            Task tx = new Task(() =>
            {
                try
                {
                    ...
                    string temp = Path.Combine(Path.GetTempPath(), string.Format("DancingMadnessPackage_{0}_{1}.zip", DateTime.Now.ToString("yyyyMMdd_HHmmss"), guid));
                    string temp2 = Path.Combine(Path.GetTempPath(), string.Format("dalamud_{0}.log", guid));
                    string temp3 = Path.Combine(Path.GetTempPath(), string.Format("DancingMadness_{0}.log", guid));
                    ...
                    string dalacfg = Path.Combine(xlpath, Path.Combine("pluginConfigs", "DancingMadness.json"));
        ...
        private void RenderFooter()
        {
            ...
            if (troubleShootingOpen == true)
            {
                ...
                if (ImGui.Begin(I18n.Translate("Troubleshooting/Title") + "##DancingMadnessTroubleshooting", ref troubleShootingOpen, flax) == true)
                {
                    UserInterface.KeepWindowInSight();
            ...
            if (ImGui.Button("GitHub") == true)
            {
                Task tx = new Task(() =>
                {
                    Process p = new Process();
                    p.StartInfo.UseShellExecute = true;
                    p.StartInfo.FileName = @"https://github.com/SneakiestBear/Dancing-Madness";
                    p.Start();
                });
                tx.Start();
            }
        ...
        private void DrawTimelineSelectorWindow()
        {
            ...
            ImGui.BeginTable("DancingMadness_TimelineSelector", 4, ImGuiTableFlags.Resizable | ImGuiTableFlags.RowBg | ImGuiTableFlags.ScrollY);
            ...
            ImGui.TableSetupColumn("##DancingMadness_TimelineSelector_actions", ImGuiTableColumnFlags.WidthFixed | ImGuiTableColumnFlags.NoSort | ImGuiTableColumnFlags.NoResize);
